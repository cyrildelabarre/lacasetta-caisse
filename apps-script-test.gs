/**
 * ATTENTION : VERSION TEST / FORMATION — a deployer dans un projet SEPARE.
 * Ecrit dans un Sheet de test auto-cree, jamais dans la prod. Aucun email.
 */

const SHEET_NAME = 'Transactions';
const TZ         = 'Europe/Paris';

// ATTENTION : DÉPLOIEMENT DE TEST / FORMATION — nécrit JAMAIS dans le Sheet de prod.
// Le classeur de test est créé automatiquement à la 1re écriture ; son ID est
// mémorisé dans les propriétés du script. Aucun email nest envoyé (TEST_MODE).
...
function getOrCreateSpreadsheet() {
  // Version TEST : crée (une fois) son propre classeur puis le réutilise.
  const props = PropertiesService.getScriptProperties();
  const id = props.getProperty('TEST_SPREADSHEET_ID');
  if (id) {
    try { return SpreadsheetApp.openById(id); } catch (e) { /* recréé ci-dessous */ }
  }
  const ss = SpreadsheetApp.create('La Casetta — CAISSE (TEST / formation)');
  props.setProperty('TEST_SPREADSHEET_ID', ss.getId());
  return ss;
}

// Renvoie lURL du classeur de test (créé si besoin).
...
function sendDailyReport() {
  if (TEST_MODE) return;  // aucun email en mode test
...
function scheduleDailyReport() {
  if (TEST_MODE) return;  // pas de declencheur demail en mode test
  ScriptApp.getProjectTriggers().forEach(t => {
    if (t.getHandlerFunction() === 'sendDailyReport') ScriptApp.deleteTrigger(t);
  });
